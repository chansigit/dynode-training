...
from cell_dynamics import DynVelocity
...
def update_config_from_hydra(cfg: DictConfig) -> dict:
    ...
    updated_config = {
        ...
        'preset_radius': cfg.evaluation.preset_radius,
...
def main(cfg: DictConfig):
    ...
    # Setup static radius for evaluation (only if evaluation is enabled)
    static_radius = None
    if CONFIG['enable_eval']:
        if CONFIG['preset_radius'] is not None:
            static_radius = CONFIG['preset_radius']
            logger.info(f"Using preset static radius: {static_radius:.4f}")
        else:
